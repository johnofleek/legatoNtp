...
)
{
    ...
    char intfName[101];
    _msgBufPtr = UnpackString( _msgBufPtr, intfName, 101 );

    bool isConnected;
    _msgBufPtr = UnpackData( _msgBufPtr, &isConnected, sizeof(bool) );
...
)
{
    ...
    _UNLOCK
    addHandlerRef = (le_data_ConnectionStateHandlerRef_t)clientDataPtr->handlerRef;
    le_mem_Release(clientDataPtr);
    _msgBufPtr = PackData( _msgBufPtr, &addHandlerRef, sizeof(le_data_ConnectionStateHandlerRef_t) );
...
)
{
    ...
    // Pack the input parameters
    _msgBufPtr = PackData( _msgBufPtr, &requestRef, sizeof(le_data_RequestObjRef_t) );
...
)
{
    ...
    // Pack the input parameters
    _msgBufPtr = PackData( _msgBufPtr, &rank, sizeof(uint32_t) );
    _msgBufPtr = PackData( _msgBufPtr, &technology, sizeof(le_data_Technology_t) );
